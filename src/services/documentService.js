...
export class DocumentService {
    ...
    async useOldVersion(documentId, versionId, userId) {
        ...
        // Update untuk documentService.js
        const hasPackageSig = version.packages && version.packages.some(
            (p) => p.package && p.package.status === 'completed'
        );
    ...
    // ==========================================
    // BAGIAN PENTING YANG DITAMBAHKAN & DIUBAH
    // ==========================================
    ...
    async getDocumentFileUrl(documentId, userId, isDownload = false) {
        ...
        // 2. Ambil versi aktif (current version)
        ...
        // 3. Tentukan nama file JIKA mode download
        let customFilename = null;
        if (isDownload) {
            const sanitizedTitle = document.title.replace(/\.pdf$/i, "").replace(/[\s/\\?%*:|"<>]/g, "_");
            // Kita bisa tambahkan penanda versi jika mau, atau biarkan nama aslinya
            customFilename = `${sanitizedTitle}.pdf`;
        }

        // 4. Minta Signed URL ke Storage Service
        // Pastikan Storage Service Anda mendukung parameter ke-3 sebagai null
    ...
    async getVersionFileUrl(documentId, versionId, userId, isDownload = false) {
        ...
        // Hanya generate custom filename jika requestnya adalah DOWNLOAD
        if (isDownload) {
        ...
        // Jika isDownload false, customFilename bernilai NULL.
        // Ini akan membuat Supabase TIDAK menambahkan '?download=' di URL.
